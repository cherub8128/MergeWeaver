...
const TOTAL_WALL_WEIGHT = WALL_VARIANTS.reduce((acc, variant) => acc + variant.weight, 0);
...
function pickTreeFor(x, y, seed=20250301){
  ...
  for (const variant of WALL_VARIANTS) {
    pick -= variant.weight;
    if (pick <= 0) { chosen = variant.src; break; }
  }

  const rot = WALL_ROTATIONS[Math.floor(random() * WALL_ROTATIONS.length)] || 0;
...
export function drawMapCoords(containerX, containerY, gridSize) {
  containerX.innerHTML = ''; containerY.innerHTML = '';
  for (let i = 0; i < gridSize; i++) {
    const xEl = document.createElement('div'); xEl.className = 'map-coord font-orbitron'; xEl.textContent = i; containerX.appendChild(xEl);
    const yEl = document.createElement('div'); yEl.className = 'map-coord'; yEl.textContent = gridSize - 1 - i; containerY.appendChild(yEl);
  }
}

export function updateUI({ gridEl, gridSize, containerEl, playerEl, targetEl, state }, renderMapItems) {
  const ro = new ResizeObserver(entries => {
    ...
    playerEl.style.transform = `translate(${state.playerPos.x * cellSize}px, ${(gridSize - 1 - state.playerPos.y) * cellSize}px)`;
    targetEl.style.setProperty('--tw-translate', `translate(${state.targetPos.x * cellSize}px, ${(gridSize - 1 - state.targetPos.y) * cellSize}px)`);
    renderMapItems(cellSize);
  });
  ro.observe(gridEl);
}

export function makeRenderMapItems({ gridEl, itemsContainer, state, gridSize }) {
  return function renderMapItems(cellSize) {
    ...
    if (state.currentStageGem && !state.currentStageGem.collected) {
      ...
      gemEl.style.transform = `translate(${gem.x * cellSize}px, ${(gridSize - 1 - gem.y) * cellSize}px)`;
      const img = document.createElement('img'); img.src = ItemSprites.gems[gem.id]; img.className = 'w-3/4 h-3/4';
    ...
    state.mapItems.forEach(item => {
      ...
      itemEl.style.transform = `translate(${item.x * cellSize}px, ${(gridSize - 1 - item.y) * cellSize}px)`;
      const img = document.createElement('img'); img.className = 'w-3/4 h-3/4';
      if (item.type === 'pouch') img.src = ItemSprites.pouch;
      else if (item.type === 'recycler') img.src = ItemSprites.recycler;
      else if (item.type === 'vector') img.src = VectorSprites.pathFromVector(item.vector.x, item.vector.y);
